...
如果你在 Codex 里工作而不是用 Claude Code，走这条路径。它不改变上面的 Claude 插件安装格式，而是把同一个 Zotero bridge 暴露成 Codex 可直接调用的命令行工具。
...
# 2) 从 git 安装 Zotron CLI / SDK。
uv tool install "git+https://github.com/dianzuan/zotron.git#subdirectory=claude-plugin/python"

# 3) 安装 Zotero 侧 XPI。
#    从最新 release 下载 zotron.xpi，或从本地 checkout 复制 claude-plugin/xpi/zotron.xpi，
#    然后在 Zotero 里安装：
#    工具 → 插件 → ⚙ → 从文件安装附加组件 → 重启。

# 4) 先验证 bridge，再让 Codex 搜索、引用、OCR 或建索引。
zotron ping
zotron search quick "数字经济" --limit 10
```

`zotron ping` 成功后，Codex 可以直接调用 `zotron`、`zotron-rag`、`zotron-ocr` 或裸 HTTP。当前还没有原生 Codex 插件包，所以在发布 `.codex-plugin/` wrapper 之前，这就是支持的 `code-cli` 路径。
