...
Codex 用户可以复用同一套 Zotero bridge，不改变上面的 Claude Code 安装路径。在 Codex marketplace 包正式发布前，可以从本仓库（或 release checkout）复制插件形态的文件到 Codex plugin/skills 目录，并安装同一个 Python CLI + XPI：

```bash
# 1) 手动安装最新 release 的 Zotero XPI，或使用当前 checkout：
#    claude-plugin/xpi/zotron.xpi
# 2) 安装 Codex skills 会调用的 CLI 命令：
uv tool install "git+https://github.com/dianzuan/zotron.git#subdirectory=claude-plugin/python"
# 3) 确保 code-cli / Codex session 能在 PATH 里找到这些命令：
zotron ping
zotron-ocr status --collection "我的文献"
zotron-rag hits "数字经济" --collection "我的文献" --output jsonl
```

后续的 `codex-plugin/` 包应镜像现有插件布局：`.codex-plugin/plugin.json`、`skills/zotero/SKILL.md`、`skills/zotero/*.md`、`agents/zotero-researcher.md` 和 `bin/zotron*`。当前阶段，Codex/code-cli 用户只需要在 Zotero 中安装 XPI、安装 Python CLI，并把 `zotron`、`zotron-ocr`、`zotron-rag` 暴露到 `PATH`。
