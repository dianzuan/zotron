---
description: Set up Zotero Bridge — verify the XPI plugin is installed and reachable, install it if missing, and confirm the bridge is live on localhost:23119.
---

# /setup — Zotero Bridge bootstrap

Run this when the user has just installed the `zotero-bridge` Claude Code plugin and needs to get the Zotero side wired up.

## Goal

End state: `system.ping` over `localhost:23119/zotero-bridge/rpc` returns `{"pong": true, ...}` and the user can ask Claude to do real Zotero work.

## Procedure

### 1. Check `uv` is available

The bundled `zotero-bridge` / `zotero-rag` / `zotero-ocr` shims invoke `uv run`. If `uv` is missing, nothing else matters.
...
```bash
curl -sf -X POST http://localhost:23119/zotero-bridge/rpc \
  -H 'Content-Type: application/json' \
  -d '{"jsonrpc":"2.0","method":"system.ping","params":{},"id":1}' \
  && echo OK || echo DOWN
```

- **OK** → the bridge is live. Run `zotero-bridge ping` to print version info, confirm to the user, and stop.
...
```bash
curl -sL https://api.github.com/repos/dianzuan/zotero-bridge/releases/latest \
...
```bash
curl -sL https://api.github.com/repos/dianzuan/zotero-bridge/releases | grep -E '"tag_name"|browser_download_url.*xpi' | head -20
```
...
curl -sL -o "$HOME/Downloads/zotero-bridge.xpi" "<URL_FROM_4A>"
ls -lh "$HOME/Downloads/zotero-bridge.xpi"
```
...
> 4. Pick `zotero-bridge.xpi` from the path I just downloaded it to
> 5. **Restart Zotero**
>
...
If still `DOWN`:
- Check Zotero's plugin pane: `Tools → Plugins` should show `Zotero Bridge` as enabled.
...
Once `system.ping` returns `OK`, run `zotero-bridge system.libraries` to print the user's library names and confirm a working bridge end-to-end. Hand off to the `zotero` skill for actual work.
